...
class GCMDevice(Device):
    ...
    def send_message(self, message, **kwargs):
        ...
        if message is not None:
            data["message"] = message
        return gcm_send_message(registration_id=self.registration_id, message=data, **kwargs)


class JPUSHDeviceManager(models.Manager):
    def get_queryset(self):
        return JPUSHDeviceQuerySet(self.model)


class JPUSHDeviceQuerySet(models.query.QuerySet):
    def send_message(self, message, **kwargs):
        from .jpush import jpush_send_bulk_message
        if self:
            reg_ids = list(self.filter(active=True).values_list('registration_id', flat=True))
            return jpush_send_bulk_message(registration_ids=reg_ids, message=message, **kwargs)


class JPUSHDevice(Device):
    device_id = HexIntegerField(verbose_name=_("Device ID"), blank=True, null=True, db_index=True,
        help_text=_("ANDROID_ID / TelephonyManager.getDeviceId() (always as hex)"))
    registration_id = models.CharField(verbose_name=_("Registration ID"), max_length=200)

    objects = JPUSHDeviceManager()

    class Meta:
        verbose_name = _("JPUSH device")
        unique_together = ('registration_id', 'user')

    def send_message(self, message, **kwargs):
        from .jpush import jpush_send_bulk_message
        reg_ids = [self.registration_id]
        return jpush_send_bulk_message(registration_id=reg_ids, data=message, **kwargs)
