...
from push.models import APNSDevice, GCMDevice, JPUSHDevice
from push.fields import hex_re
from push.fields import UNSIGNED_64BIT_INT_MAX_VALUE
...
class JPUSHDeviceSerializer(ModelSerializer):
    device_id = HexIntegerField(
        help_text="ANDROID_ID / TelephonyManager.getDeviceId() (e.g: 0x01)",
        style={'input_type': 'text'},
        required=False
    ...
    class Meta(DeviceSerializerMixin.Meta):
        ...
        #     'registration_id': {
        #         'validators': [
        #             UniqueValidator(queryset=JPUSHDevice.objects.all())
        #         ]
        #     }
        # }
...
class DeviceViewSetMixin(object):
    ...
    def perform_create(self, serializer):
        if self.request.user.is_authenticated():
            '''由于registration_id和user是unique_together,所以如果已经添加过则不再添加'''
            if JPUSHDevice.objects.filter(user=self.request.user, registration_id=serializer.validated_data["registration_id"]).exists() \
                    or APNSDevice.objects.filter(user=self.request.user, registration_id=serializer.validated_data["registration_id"]).exists():
                return
            else:
...
class JPUSHDeviceViewSet(DeviceViewSetMixin, ModelViewSet):
    authentication_classes = (SessionAuthentication, BasicAuthentication, TokenAuthentication)
    permission_classes = (IsAuthenticated,)

    queryset = JPUSHDevice.objects.all()
    serializer_class = JPUSHDeviceSerializer


class JPUSHDeviceAuthorizedViewSet(AuthorizedMixin, JPUSHDeviceViewSet):
    pass